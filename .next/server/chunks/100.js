...
const webcrypto_isCryptoKey = external_util_.types.isCryptoKey ? (key)=>external_util_.types.isCryptoKey(key) : (key)=>false;
...
function get_named_curve_setCurve(keyObject, curve) {
...
class DumbAsn1Encoder {
    ...
    zero() {
        this.elements.push(bTagInteger, Buffer.from([
            0x01
        ]), bZero);
        this.length += 3;
    }
...
const flags_jwkExport = major >= 16 || major === 15 && minor >= 9;
const flags_jwkImport = major >= 16 || major === 15 && minor >= 12;
        ...
        case "EC":
            {
                ...
                if (isPrivate) {
                    ...
                    enc$3.bitStr(pub);
                    const f2 = enc$3.end(Buffer.from([
                        0xa1
                    ]));
                    enc$2.add(f2);
                    const f = enc$2.end();
                    ...
                    enc$4.add(f);
                    const f3 = enc$4.end(Buffer.from([
                        0x04
                    ]));
                    enc.add(f3);
                    const der = enc.end();
...
const generate_generate = (0,external_util_.promisify)(external_crypto_.generateKeyPair);
