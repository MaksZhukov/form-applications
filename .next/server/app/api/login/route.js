...
const POST = async (request)=>{
    const { data: { password, email } } = await request.json();
    const { OrganizationModel } = await (0,db/* initialize */.j)();
    const userData = (await OrganizationModel.findOne({
        where: {
            email
        }
    }))?.toJSON();
    if (!userData) {
        return new next_response/* default */.Z("wrong email", {
            status: 400
        });
    }
    if (!await external_bcrypt_default().compare(password, userData.password)) {
        return new next_response/* default */.Z("wrong password", {
            status: 400
        });
    }
    const token = await (0,jwt/* sign */.Xx)({
        id: userData.id,
        email,
        password,
        role: userData.role
    ...
    try {
        if (await OrganizationModel.update({
            token
        }, {
            where: {
                id: userData.id
...
;// CONCATENATED MODULE: ./node_modules/next/dist/build/webpack/loaders/next-app-loader.js?page=%2Fapi%2Flogin%2Froute&name=app%2Fapi%2Flogin%2Froute&pagePath=private-next-app-dir%2Fapi%2Flogin%2Froute.ts&appDir=C%3A%5CUsers%5Cmaks_%5CDesktop%5Cform-applications%5Csrc%5Capp&appPaths=%2Fapi%2Flogin%2Froute&pageExtensions=tsx&pageExtensions=ts&pageExtensions=jsx&pageExtensions=js&basePath=&assetPrefix=&nextConfigOutput=&preferredRegion=&middlewareConfig=e30%3D!
    ...
    const options = {"definition":{"kind":"APP_ROUTE","page":"/api/login/route","pathname":"/api/login","filename":"route","bundlePath":"app/api/login/route"},"resolvedPagePath":"C:\\Users\\maks_\\Desktop\\form-applications\\src\\app\\api\\login\\route.ts","nextConfigOutput":""}
...
var __webpack_exports__ = __webpack_require__.X(0, [697,501,368,100,486], () => (__webpack_exec__(72777)));
